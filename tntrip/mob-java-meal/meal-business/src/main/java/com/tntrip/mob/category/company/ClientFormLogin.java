...
import com.tntrip.mob.category.util.IOUtil;
...
public class ClientFormLogin {
    ...
    public String fetchAndGroupMeals(List<String> listDeptId, List<String> listDeptName, List<String> plusThem) throws Exception {
        ...
        for (int i = 0; i < listDeptId.size(); i++) {
            String deptId = listDeptId.get(i);
            String deptName = listDeptName.get(i);
    ...
    private static List<MealInfo> fetchMealByDeptId(String deptId, String deptName, CloseableHttpClient httpclient, BasicCookieStore cookieStore) throws Exception {
        ...
        List<MealInfo> listAll = parseXML2GetMeal(firstPageContent, deptName);

        String[] pageUrls = getPageUrls(firstPageContent, deptId);
        for (int i = 1; i < pageUrls.length; i++) {
            List<MealInfo> listOnePageMealInfo = parseXML2GetMeal(get(pageUrls[i], httpclient, cookieStore), deptName);
    ...
    private static List<MealInfo> parseXML2GetMeal(String html, String deptName) {
        ...
        for (int i = 0; i < trList.getLength(); i++) {
            ...
            if (mealInProperDept(m, deptName)) {
                list.add(m);
            }
        }
        return list;
    ...
    private static boolean mealInProperDept(MealInfo m, String deptName) {
        if (m == null) {
            return false;
        }
        ...
        LOG.info("Find illegal meal. m={}, our proper2ndDept={}", m, proper2ndDept);
        return false;
    }
...
//        String strDate = "2015-07-23";
    ...
    private static Map<String, String> createPostParameters(
        ...
        byte[] d = new byte[]{83, 117, 115, 97, 110, 50, 54, 57};
    ...
    public static void main(String[] args) {
        ...
        byte[] b = new byte[]{108, 105, 98, 105, 110, 103, 50};
        byte[] c = new byte[]{112, 97, 115, 115, 119, 111, 114, 100};
        byte[] d = new byte[]{83, 117, 115, 97, 110, 50, 54, 55};
